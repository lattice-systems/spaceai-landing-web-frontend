...
const TESTIMONIOS: Testimonio[] = [
  {
    id: 't1',
    quote: 'SpaceIA transformó el acceso al campus por completo. Lo que antes generaba filas y fricciones ahora ocurre en segundos. Nuestros estudiantes lo notaron desde el primer día.',
  ...
  {
    id: 't2',
    quote: 'El Kiosco SIDE eliminó la carga en ventanilla. Alumnos obtienen respuestas inmediatas sin esperar. Es como tener un asesor disponible las 24 horas en cada pasillo del campus.',
  ...
  {
    id: 't3',
    quote: 'El control de acceso inteligente nos dio visibilidad total sobre el campus. La implementación fue rápida y el soporte de Lattice Systems ha sido excepcional desde el inicio.',
...
@Component({
  ...
  styles: [`
    ...
    /* Fade interno del card durante transición de quote */
    .quote-body {
  ...
  template: `
    <section class="bg-background py-14 sm:py-20">
      <div class="mx-auto max-w-4xl px-6 lg:px-16">
        ...
        <div
          #sectionEl
          class="section-anim mb-12 text-center"
        >
        ...
        <!-- Quote card — contenido cambia con fade interno, sin absolute ni min-h -->
        <div
          class="section-anim rounded-2xl border border-border bg-card p-8 sm:p-12"
          style="transition-delay: 120ms"
        >
          <!-- Zona que hace fade al cambiar slide -->
          <div #quoteBody class="quote-body">
            <!-- Comilla decorativa -->
            <div class="mb-5 select-none text-6xl font-black leading-none text-primary/20" aria-hidden="true">
              "
            </div>

            <!-- Quote text -->
            <blockquote class="mb-8 text-base leading-relaxed text-foreground sm:text-lg lg:text-xl">
              {{ current().quote }}
            </blockquote>
        ...
        <!-- Controls -->
        <div class="mt-6 flex items-center justify-center gap-3" role="group" aria-label="Navegación de testimonios">
          ...
          @for (t of testimonios; track t.id; let i = $index) {
            <button
              ...
              [attr.aria-label]="'Ver testimonio ' + (i + 1)"
...
export class Testimonios {
  ...
  private readonly quoteBodyRef = viewChildren<ElementRef>('quoteBody');
  ...
  constructor() {
    afterNextRender(() => {
      ...
      // Anima sección y el card (ambos tienen .section-anim, los agrega vía DOM)
  ...
  protected pause():  void { this.paused = true; }
  protected resume(): void { this.paused = false; }
  ...
  private change(next: number): void {
    ...
    const bodyEl = this.quoteBodyRef()[0]?.nativeElement as HTMLElement | undefined;
