      ...
      <hlm-dialog-content *hlmDialogPortal class="w-full sm:max-w-2xl">
        ...
        <div class="grid gap-4 py-2">
          <div class="flex flex-wrap items-center gap-2">
            <app-status-chip
              [label]="statusLabel(selectedQuote()?.status ?? '')"
              [chip]="statusChip(selectedQuote()?.status ?? '')"
            />
  ...
  private reload(): void {
    ...
    this.quotesService
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          this.loading.set(false);
          toast.error(this.extractError(err, 'No se pudo cargar la lista de cotizaciones.'));
        },
      });
  }
}
