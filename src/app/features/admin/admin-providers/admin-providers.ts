...
export class AdminProviders {
  ...
  protected activateOne(provider: ProviderResponse): void {
    this.providersService.activate(provider.id).subscribe({
      next: () => this.reload(),
      error: (err: HttpErrorResponse) =>
        toast.error(this.extractError(err, 'No se pudo activar el proveedor.')),
    });
  }
  ...
  protected confirmDeleteOne(): void {
    ...
    this.providersService.remove(provider.id).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.deleteConfirmRef.close();
        toast.error(this.extractError(err, 'No se pudo eliminar el proveedor.'));
      },
    });
  }
  ...
  private reload(): void {
    ...
    this.providersService
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          this.loading.set(false);
          toast.error(this.extractError(err, 'No se pudo cargar la lista de proveedores.'));
        },
      });
  }
}
